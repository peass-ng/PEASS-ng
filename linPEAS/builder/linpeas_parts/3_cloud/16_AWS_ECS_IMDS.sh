# Title: Cloud - AWS ECS IMDS Exposure
# ID: CL_AWS_ECS_IMDS
# Author: HT Bot
# Last Update: 29-12-2025
# Description: Detect IMDS exposure paths for ECS tasks running on EC2 and highlight missing hardening
# License: GNU GPL
# Version: 1.0
# Functions Used: print_2title, print_3title
# Global Variables: $aws_ecs_metadata_uri, $is_aws_ecs
# Initial Functions: check_aws_ecs
# Generated Global Variables: $should_check_ecs_imds, $ecs_task_metadata, $launch_type, $network_modes, $imds_tool, $imds_token, $imds_roles, $first_role, $imds_http_code, $ecs_block_line, $ecs_host_line, $iptables_cmd, $docker_rules, $aws_exec_env
# Fat linpeas: 0
# Small linpeas: 1


should_check_ecs_imds="No"
if [ "$is_aws_ecs" = "Yes" ] || [ -f "/etc/ecs/ecs.config" ] || [ -d "/var/lib/ecs" ]; then
    should_check_ecs_imds="Yes"
fi

if [ "$should_check_ecs_imds" = "Yes" ]; then
    print_2title "ECS on EC2 IMDS Exposure"

    if [ "$is_aws_ecs" = "Yes" ]; then
        printf "Detected ECS metadata variables (likely running inside an ECS task).\n"
        ...
        if [ "$aws_ecs_metadata_uri" ]; then
            if command -v curl >/dev/null 2>&1; then
                ecs_task_metadata=$(curl -s --connect-timeout 2 --max-time 2 "$aws_ecs_metadata_uri/task" 2>/dev/null)
            elif command -v wget >/dev/null 2>&1; then
                ecs_task_metadata=$(wget -q -O - --timeout=2 --tries=1 "$aws_ecs_metadata_uri/task" 2>/dev/null)
            fi
        fi
    ...
    print_3title "IMDS reachability from this context"
    imds_token=""
    imds_roles=""
    imds_http_code=""
    imds_tool=""
    ...
    if [ "$imds_tool" = "curl" ]; then
        ...
        if [ "$imds_token" ]; then
            ...
            case "$imds_http_code" in
                000|"")
                    printf "[i] IMDS endpoint did not respond (likely blocked via hop-limit, DOCKER-USER drops or ECS agent settings).\n"
                    ;;
                401)
    ...
    print_3title "Host/agent configuration hints"
    ...
    print_3title "DOCKER-USER metadata firewall"
    iptables_cmd=""
    ...
    if [ "$iptables_cmd" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if echo "$docker_rules" | grep -q "169\\.254\\.169\\.254"; then
                ...
                echo "    -> No DOCKER-USER rule drops 169.254.169.254 traffic (bridge tasks rely on hop limit or custom firewalling)."
            fi
        else
